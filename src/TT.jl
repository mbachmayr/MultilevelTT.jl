...
#--------------------------------------------------------------
# Basic functionality of tensor train format
module TT

const Tensor3 = Array{Float64,3}
const Tensor4 = Array{Float64,4}

import Base.scale!, Base.reverse, Base.transpose, Base.norm, Base.dot
...
function Tensor4(n1::Int64, n2::Int64, n3::Int64, n4::Int64)
  return Tensor4((n1,n2,n3,n4))
end

function TensorMatrix(n::Vector{Tuple{Int64,Int64}})
  m = Vector{Tensor4}(length(n))
  m[1] = Tensor4(1, n[1]..., 0)
  for i = 2:(length(m)-1)
    m[i] = Tensor4(0, n[i]..., 0)
  end
...
function svd!(t::Tensor)
  if minimum(ranks(t)) > 0
    rightOrth!(t)
    svlist = Array{Vector{Float64}}(length(t)-1)
    for i = length(t):-1:2
      s = size(t[i])
